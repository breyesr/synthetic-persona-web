// src/app/api/personas/construction/route.ts
import { NextResponse } from "next/server";
import path from "path";
import { promises as fs } from "fs";

const DATA_DIR = path.join(process.cwd(), "data", "personas", "construction");
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        options.push({ id: `construction/${base}`, name, role });
      } catch {
        // skip invalid JSON
      }
    }
    ...
    const message =
      err instanceof Error ? err.message : "Failed to load construction personas";
